...
Your task is to create a function that takes a string of two words, separated by a space: words and returns a spoonerism of those words in a string,
 as in the above example. A "word" in the context of this kata can contain any of the letters A through Z in upper or lower case, and the numbers 0 to 9.
  Though spoonerisms are about letters in words in the domain of written and spoken language, numbers are included in the inputs for the random test
   cases and they require no special treatment.
*/
function spoonerize(string) {
  let words = string.split(" ");
  let first = words[0].split("");
  let second = words[1].split("");
  [first[0], second[0]] = [second[0], first[0]];
  let result = first.join("") + " " + second.join("");
  return result;
}
